...
class CreateClient extends Component {
              ...
              <Grid.Row>
                <Grid.Col sm={6} md={2}>
                  <PlatformItem
                    title="Android App"
                    id="platform-android"
                    inclass="fa fa-android"
                    itemSelected={this.selectPlatform}
                    selected={clientType === PLATFORM_ANDROID}
                  />
                </Grid.Col>
                <Grid.Col sm={6} md={2}>
                  <PlatformItem
                    ...
                    id="platform-cordova"
                    itemSelected={this.selectPlatform}
                    selected={clientType === PLATFORM_CORDOVA}
                  >
                    <span>
                ...
                <Grid.Col sm={6} md={2}>
                  <PlatformItem
                    ...
                    inclass="fa fa-apple"
                    itemSelected={this.selectPlatform}
                    selected={clientType === PLATFORM_IOS}
                  />
                </Grid.Col>
                <Grid.Col sm={6} md={2}>
                  <PlatformItem
                    ...
                    id="platform-xamarin"
                    itemSelected={this.selectPlatform}
                    selected={clientType === PLATFORM_XAMARIN}
                  >
                    <span>
  ...
  selectPlatform = state => {
    ...
    switch (state.id) {
      case 'platform-android':
        selectedPlatform = PLATFORM_ANDROID;
        break;
      case 'platform-ios':
        selectedPlatform = PLATFORM_IOS;
        break;
      case 'platform-cordova':
        selectedPlatform = PLATFORM_CORDOVA;
        break;
      case 'platform-xamarin':
        selectedPlatform = PLATFORM_XAMARIN;
        break;
      default:
        throw new Error(`unsupported platform ${state.id}`);
    }
    this.setState({
      activeStepIndex: WIZARD_CONFIGURE_CLIENT,
      clientConfiguration: { [CREATE_CLIENT_TYPE]: selectedPlatform }
    });
  };
  ...
  open = () => {
    if (this.state.resetOnStart) {
      // resets the state if it was already there
  ...
  configureClient = state => {
    ...
    newState.validated =
      state.validation &&
      state.validation[CREATE_CLIENT_NAME] &&
      state.validation[CREATE_CLIENT_APP_ID] &&
      !Object.values(state.validation).find(v => v === 'error'); // check if everything was filled in and validated
  ...
  renderClientSpecificForm = () => {
    switch (this.state.clientConfiguration[CREATE_CLIENT_TYPE]) {
      case PLATFORM_ANDROID:
        return (
          <CreateAndroidClient
            configureClient={this.configureClient}
            clientConfiguration={this.state.clientConfiguration}
          />
        );
  ...
  nextStepDisabled = () =>
    (this.state.activeStepIndex === WIZARD_SELECT_PLATFORM && this.state.platform === undefined) ||
    (this.state.activeStepIndex === WIZARD_CONFIGURE_CLIENT && !this.state.validated);
